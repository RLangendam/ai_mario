...
class EvolutionaryAlgorithm:
    ...
    def run(self, agent, gyms, size=100, generation_count=10, selection_factor=0.04, agent_timeout=10000,
            survival_factor=0.5, sigma=0.2):
        population = self.initialize_population(agent, size)
        for generation in range(generation_count):
            fitnesses = list(map(self.compute_fitness(gyms[0], agent_timeout), population))  # adjust
